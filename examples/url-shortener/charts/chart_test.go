...
// A service that authenticates with a certificate holds no password at all.
//
// Not "does not use one" — does not have one. The point of the mode is
// that there is nothing to leak, nothing to rotate and nothing to print,
// so a DATABASE_PASSWORD left in the environment beside the certificate
// would undo it while every test about certificates still passed.
//
// The migration is the deliberate exception: it uses the OWNER role, whose
// credential the database operator issues with the bootstrap, and it is
// the only thing that ever holds it.
func TestACertificateRoleCarriesNoPassword(t *testing.T) {
	out, err := render(t, defaults(
		"--set", "database.app.auth=certificate",
		"--set", "database.app.certificateSecret=pg-runtime",
	)...)
	if err != nil {
		t.Fatalf("the chart does not render: %v\n%s", err, out)
	}

	withPassword, withCert := map[string]bool{}, map[string]bool{}
	for _, doc := range strings.Split(out, "\n---") {
		var m map[string]any
		if err := yaml.Unmarshal([]byte(doc), &m); err != nil || m == nil {
			continue
		}
		if m["kind"] != "Deployment" && m["kind"] != "Job" {
			continue
		}
		...
		for _, c := range containers {
			...
			for _, e := range asList(container["env"]) {
				...
				if env["name"] == "DATABASE_PASSWORD" {
					withPassword[name] = true
				}
			}
			for _, v := range asList(container["volumeMounts"]) {
				mount, _ := v.(map[string]any)
				if mount["name"] == "database-identity" {
					withCert[name] = true
				}
			}
		}
	...
	for _, name := range []string{"example-redirect", "example-stat", "example-urls"} {
		if !withCert[name] {
			t.Errorf("%s reaches the database but mounts no certificate", name)
		}
		if withPassword[name] {
			t.Errorf("%s carries a password beside its certificate — the mode exists so that there is none", name)
		}
	}

	// The exception, and it is the owner's.
	if !withPassword["example-migrate"] {
		t.Error("the migration lost the owner's credential; it is the only thing that holds it")
	}

	// verify-full, not require: a client certificate proves the client to
	// the server and nothing in the other direction.
	if !strings.Contains(out, "sslmode=verify-full") {
		t.Error("the connection does not verify the server it presents a certificate to")
	}
}

// asList tolerates a nil or absent sequence.
